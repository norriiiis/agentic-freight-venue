...
const ROLES: Record<string, string> = { "witness-1": "WITNESS", "northline-broker-agent": "BROKER", "prairie-wind-carrier-agent": "CARRIER", "blue-mesa-carrier-agent": "CARRIER2", "spoof-carrier-agent": "SPOOFER", "quikhaul-agent": "quikhaul", venue: "venue" };
...
async function runScenario(sc: Scenario, index: number): Promise<{ res: ScenarioResult; pass: boolean; workspace: string }> {
  ...
  try {
    ...
    const wire = renderWire(await h.venue.messages(), ROLES, ["happy-path", "non-convergence", "exposure-mid-negotiation", "insurance-lapsed", "negotiation-timeout", "key-rotation", "venue-key-rotation", "root-key-rotation", "status-timestamping"].includes(sc.id) ? res.taskId : undefined);
    if (wire.length) {
      console.log("\n  wire (venue log):");
