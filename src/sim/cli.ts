...
const ROLES: Record<string, string> = { "northline-broker-agent": "BROKER", "prairie-wind-carrier-agent": "CARRIER", "spoof-carrier-agent": "SPOOFER", "quikhaul-agent": "quikhaul", venue: "venue" };
...
async function runScenario(sc: Scenario, index: number): Promise<{ res: ScenarioResult; pass: boolean; workspace: string }> {
  ...
  try {
    ...
    const wire = renderWire(await h.venue.messages(), ROLES, sc.id === "happy-path" || sc.id === "non-convergence" || sc.id === "exposure-mid-negotiation" || sc.id === "insurance-lapsed" ? res.taskId : undefined);
    if (wire.length) {
      console.log("\n  wire (venue log):");
