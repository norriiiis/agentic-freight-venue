...
async function runScenario(sc: Scenario, index: number): Promise<{ res: ScenarioResult; pass: boolean; workspace: string }> {
  ...
  try {
    ...
    const wire = renderWire(await h.venue.messages(), ROLES, ["happy-path", "non-convergence", "exposure-mid-negotiation", "insurance-lapsed", "negotiation-timeout", "key-rotation", "venue-key-rotation", "root-key-rotation", "status-timestamping", "witness-equivocation", "witness-collusion", "delayed-recording"].includes(sc.id) ? res.taskId : undefined);
    if (wire.length) {
      console.log("\n  wire (venue log):");
