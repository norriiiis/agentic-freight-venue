...
export const insurerKeyBinding: Scenario = {
  ...
  async run({ h, say }) {
    ...
    await h.rotateFilerKey({ insurerId: gpm.insurerId, publicKey: k2.publicJwk, kid: k2.kid, licensedBy: regulator.license({ naicCode: gpm.naicCode, legalName: gpm.insurerName, publicKey: k2.publicJwk }) });
    await h.revokeFilerKey({ insurerId: gpm.insurerId, kid: gpm.kp.kid, revokedAt, reason: "ROTATION", authorization: gpm.selfRevoke(gpm.kp, { kid: gpm.kp.kid, revokedAt, reason: "ROTATION" }) });
    ...
    say(`part 2: ${gpm.insurerId} rotates at the registries — a successor key ${k2.kid.slice(0, 12)}… licensed by ${regulator.regulatorId}, and revokes ${gpm.kp.kid.slice(0, 12)}… (rotation, ${revokedAt.slice(11, 19)}Z); ${c!.registryId} stopped syncing yet claims a current sync. A COI signed with the old key → ${r2.reasonCode}: ${e2.error}`);
