...
export const REASONS = {
  ...
  INSURANCE_NOT_ASSURED_THROUGH_DELIVERY: "No word from the insurer signed before pickup can assure coverage through delivery: transit outrun the statutory notice period, so the origin cannot vouch for this load",
  ...
  REGULATOR_KEY_UNTRUSTED: "The regulator's attestation is signed by a key not reachable from the pinned regulator key by pre-committed rotations, or declared compromised as of a time before the signature; the regulator did not speak",
  REGULATOR_ROTATION_UNAUTHORIZED: "A regulator's key rotates only to the successor it pre-committed to, signed by that successor; the current key cannot appoint a different one, and a registry cannot be given a new regulator root by anyone",
  FILER_UNLICENSED: "A filer's key is registered only against its regulator's signed word that this licensed insurer files under this key; none was given, it does not verify, or it binds another name or key — the registry onboarded (or would onboard) the key on nobody's authority",
  FILER_ROTATION_UNAUTHORIZED: "A filer's successor key must be licensed by its regulator; the current key may revoke itself but cannot appoint its replacement — a thief holding it gains nothing",
