 ...
 * --insurer-key f[,id[,name]]  pin an insurer (repeatable): the party's own insurer's word must verify under it and, given
 *                        the name it files under, be OF RECORD — the registry's filing must name that insurer and policy
...
import type { InsurerAttestation, InsurerKey, RegistryAttestation, RegistryKey } from "../protocol/registry";
...
console.log(`  insurer    ${artifact.insurance?.carrier ? `carrier's own insurer's word embedded (${artifact.insurance.carrier.insurerId}, as of ${artifact.insurance.carrier.asOf}${artifact.insurance.carrier.cancellation ? `, cancellation disclosed effective ${artifact.insurance.carrier.cancellation.effectiveDate}` : ""}${artifact.insurance.carrier.undertaking ? `, UNDERTAKING ${artifact.insurance.carrier.undertaking}` : ", certificate only — no undertaking"}); key ${insurerKeys.length ? "PINNED" : "not pinned — signature unchecked"}${artifact.insurance.renewal ? `; CONDITIONAL: renewal signed ≥ ${artifact.insurance.renewal.earliestSignedAt} due by ${artifact.insurance.renewal.dueBy}` : ""}` : requireInsurerAttestation ? "NONE embedded — required" : "none embedded"}${renewals.length ? `; ${renewals.length} renewal(s) supplied` : ""}${currentInsurerAttestations.length ? `; ${currentInsurerAttestations.length} current insurer attestation(s) supplied` : ""}`);
