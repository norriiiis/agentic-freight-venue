...
console.log(`  insurer    ${artifact.insurance?.carrier ? `carrier's own insurer's word embedded (${artifact.insurance.carrier.insurerId}, as of ${artifact.insurance.carrier.asOf}${artifact.insurance.carrier.cancellation ? `, cancellation disclosed effective ${artifact.insurance.carrier.cancellation.effectiveDate}` : ""}); key ${insurerKeys.length ? "PINNED" : "not pinned — signature unchecked"}${artifact.insurance.renewal ? `; CONDITIONAL: renewal signed ≥ ${artifact.insurance.renewal.earliestSignedAt} due by ${artifact.insurance.renewal.dueBy}` : ""}` : requireInsurerAttestation ? "NONE embedded — required" : "none embedded"}${renewals.length ? `; ${renewals.length} renewal(s) supplied` : ""}${currentInsurerAttestations.length ? `; ${currentInsurerAttestations.length} current insurer attestation(s) supplied` : ""}`);
