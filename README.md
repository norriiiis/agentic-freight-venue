...
**A convincing refusal is the product.** Fourteen adversarial scenarios each fail with a machine-readable reason, the component that refused, the evidence it relied on, and whether the guarantee would have paid.
...
npm test                # 84 tests incl. the no-shared-state proof and all scenarios (~60s)
```
...
| `src/sim/` | Process harness (spawn, kill, restart, re-provision an agent with a new key; acts as each principal's key-management system), fixtures, fifteen scenarios, transcript renderer, CLI. |
...
- Credential lifecycle: issuance against registry evidence, expiry, revocation, principal-authorized rotation with lineage / grace / compromise time, three-layer verification on every exchange, live re-check at tender / commit / pre-pickup, published status list for offline verifiers.
...
4. **Key custody.** Agent and venue private keys are JWK files on disk. The venue key must move to an HSM/KMS. The *rotation protocol* exists (principal-authorized, with lineage, grace and compromise semantics), but the principal key that authorizes it is generated by the simulator; in production it lives in the principal's own KMS/SSO-backed signing flow, and the venue key itself has no rotation path yet.
5. **Status propagation.** The status list (revocations + rotations, with `compromisedAt`) is published at a well-known URL and the artifact verifier consumes it, which is what lets a verifier months later judge a signature *at signing time*. It is not yet signed or timestamped as a whole (OCSP-style), so an offline verifier must trust the copy it was handed.
