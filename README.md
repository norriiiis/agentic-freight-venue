...
**A convincing refusal is the product.** Sixteen adversarial scenarios each fail with a machine-readable reason, the component that refused, the evidence it relied on, and whether the guarantee would have paid.
...
npm test                # 92 tests incl. the no-shared-state proof and all scenarios (~80s)
```
...
| `src/sim/` | Process harness (spawn, kill, restart, re-provision an agent with a new key; acts as each principal's key-management system and as the venue operator's root-key custody, including the pre-committed next root), fixtures, seventeen scenarios, transcript renderer, CLI. |
