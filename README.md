...
**A convincing refusal is the product.** Seventeen adversarial scenarios each fail with a machine-readable reason, the component that refused, the evidence it relied on, and whether the guarantee would have paid.
...
npm test                # 97 tests incl. the no-shared-state proof and all scenarios (~90s)
```
...
npm run verify -- <artifact.json> --venue-root <venue-root-public.jwk.json> --ledger <ledger.jsonl> [--key-history …] [--status-list …] [--witness-key …] [--as-of …]
...
| `src/witness/` | An independent ledger witness: its own process, key and clock; cosigns each new head only if it extends the last one it cosigned; records forks. What turns the venue's "as of" into a third party's. |
| `src/sim/` | Process harness (spawn, kill, restart, re-provision an agent with a new key, start witnesses; acts as each principal's key-management system and as the venue operator's root-key custody, including the pre-committed next root), fixtures, eighteen scenarios, transcript renderer, CLI. |
...
**Witnessed publication (third-party time).** Credential revocations and supersessions are `CREDENTIAL_STATUS` entries on the ledger, so the published status list and key history are *projections of a chain head*, not free-standing documents, and each carries `head` plus the latest head an independent **witness** cosigned. A witness (`src/witness/`) is a separate process with its own key and clock: each poll it fetches the head, fetches the entries since the last head it cosigned and verifies the hash chain links to the new one — a rolled-back or forked ledger cannot produce that (`LEDGER_FORK_DETECTED`, refused) — then signs `{venueId, seq, hash, at}` and hands the receipt to the venue for publication. Witness keys are learned out of band, like CT log keys; the venue accepts receipts only from registered witnesses and cannot mint them. A verifier that pins witnesses (`--witness-key`) then knows the time **W** up to which the lists are complete — the witness's time, not the venue's `asOf` — and requires W ≥ the moment being judged minus a tolerance (`--as-of`, `--max-staleness-ms`; default now, 15 min): otherwise the verdict is `STATUS_STALE` / `STATUS_NOT_WITNESSED` — "signatures genuine, status uncertain" — never "trusted". With the ledger file it also checks the list is complete up to the witnessed head. What this does not solve: a venue can *delay* recording a revocation; no witness can cosign what it was never shown. The guarantee is "complete as of W", never "nothing has happened". `protocol/witness.ts`, `witness/service.ts`, `ledger/artifact.ts → verifyArtifact()`.
...
- Third-party time for the published lists: status changes on the ledger, independent witness processes that cosign heads and detect forks, verifier freshness semantics (`STATUS_STALE` rather than silent trust).
...
5. **Witness ecosystem.** The freshness guarantee is only as good as the witnesses: in the prototype there is one, run by the simulator, polling every few hundred milliseconds. Production needs several independent witnesses (an insurer, an industry body, a public timestamping service, the counterparties themselves), a policy for how many must agree, gossip between them so they detect a venue that shows different heads to different witnesses (equivocation), and a public archive of receipts that survives the venue. A venue can still *delay* recording a revocation; witnessing bounds the damage to the detection window, it does not remove it.
