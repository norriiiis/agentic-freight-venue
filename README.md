...
**A convincing refusal is the product.** Nineteen adversarial scenarios each fail with a machine-readable reason, the component that refused, the evidence it relied on, and whether the guarantee would have paid.
...
npm test                # 104 tests incl. the no-shared-state proof and all scenarios (~110s)
```
...
| `src/witness/` | An independent ledger witness: the witnessing core (`protocol/witness-core.ts`, shared with agents) as its own process with its own key and clock; cosigns each new head only if it extends the last one it cosigned; remembers every hash it verified; gossips at positions both peers have verified and turns a disagreement into a self-contained equivocation proof, then halts. What turns the venue's "as of" into a third party's, and a split view into evidence. |
| `src/sim/` | Process harness (spawn, kill, restart, re-provision an agent with a new key, start witnesses with peers and make them collude; acts as each principal's key-management system, each principal's choice of witnesses, and the venue operator's root-key custody including the pre-committed next root), fixtures, twenty scenarios, transcript renderer, CLI. |
