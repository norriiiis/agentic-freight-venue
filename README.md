...
**A convincing refusal is the product.** Eighteen adversarial scenarios each fail with a machine-readable reason, the component that refused, the evidence it relied on, and whether the guarantee would have paid.
...
npm test                # 100 tests incl. the no-shared-state proof and all scenarios (~100s)
```
...
npm run verify -- <artifact.json> --venue-root <venue-root-public.jwk.json> --ledger <ledger.jsonl> [--key-history …] [--status-list …] [--witness-key …] [--min-witnesses N] [--equivocation-proof …] [--as-of …]
...
| `src/witness/` | An independent ledger witness: its own process, key and clock; cosigns each new head only if it extends the last one it cosigned; remembers every hash it verified; gossips receipts with peer witnesses and turns a disagreement into a self-contained equivocation proof, then halts. What turns the venue's "as of" into a third party's, and a split view into evidence. |
| `src/sim/` | Process harness (spawn, kill, restart, re-provision an agent with a new key, start witnesses with peers; acts as each principal's key-management system and as the venue operator's root-key custody, including the pre-committed next root), fixtures, nineteen scenarios, transcript renderer, CLI. |
...
**Split views (equivocation).** One witness checks only that the venue's chain extends *its own* view, so a venue can keep two books and show each witness a consistent one. Two mechanisms close this. **Gossip**: witnesses exchange their latest receipts; a peer's receipt for a seq this witness verified with a different hash is, paired with this witness's own receipt for that seq, a self-contained **equivocation proof** — two witness-signed receipts, same venue, same position, different hashes — verifiable with the witness keys alone. On proof a witness halts (never cosigns that venue again) and pushes the proof to its peers. **Quorum**: a verifier requires `--min-witnesses k` distinct pinned witnesses on the *same* head, and takes the k-th latest receipt time as W; to publish a fresh head with a quorum the venue must show one head to k witnesses, who gossip. Any valid proof for a venue voids everything it publishes (`VENUE_EQUIVOCATION`). What this does not solve: collusion — a venue that controls k witnesses meets the quorum and their gossip is silent; the defence is who the witnesses are (the insurer, an industry body, the counterparties themselves), not the protocol. And delay: a venue can *decline to record* a revocation; no witness can cosign, or disagree about, what it was never shown. The guarantee is "complete as of W", never "nothing has happened". `protocol/witness.ts`, `witness/service.ts`, `ledger/artifact.ts → verifyArtifact()`.
...
- Third-party time for the published lists: status changes on the ledger, independent witness processes that cosign heads, detect forks, gossip, and produce equivocation proofs; verifier freshness and quorum semantics (`STATUS_STALE`, `WITNESS_QUORUM_NOT_MET`, `VENUE_EQUIVOCATION` rather than silent trust).
...
5. **Witness ecosystem.** The protocol is there — cosigning, fork detection, gossip, equivocation proofs, quorum — but the witnesses are two processes run by the simulator. Production needs witnesses that are actually independent of the venue and of each other (an insurer, an industry body, a public timestamping service, the counterparties themselves), a public archive of receipts and proofs that survives the venue, and a policy for k. Collusion of k witnesses with the venue defeats quorum and silences gossip; only the choice of witnesses defends against that. A venue can still *delay* recording a revocation; witnessing bounds the damage to the detection window, it does not remove it.
